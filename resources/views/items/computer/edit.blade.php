<x-admin.wrapper>
    ...
    <div class="w-full py-2 bg-white overflow-hidden">
        ...
        <div class="py-2">
        <x-admin.form.label for="power" class="{{$errors->has('power') ? 'text-red-400' : ''}}">{{ __('Power') }}</x-admin.form.label>

        <x-admin.form.input id="power" class="{{$errors->has('power') ? 'border-red-400' : ''}}"
                            type="text"
                            name="power"
                            value="{{ old('power', $computer->power) }}"
                            />
        </div>
