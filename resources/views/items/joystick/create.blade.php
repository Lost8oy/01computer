<x-admin.wrapper>
    ...
    <div>
        <x-admin.breadcrumb href="{{route('joystick.index')}}" title="{{ __('Add Joystick') }}">{{ __('<< Back to Joysticks') }}</x-admin.breadcrumb>
        <x-admin.form.errors />
    </div>
    <div class="w-full py-2 bg-white overflow-hidden">

        <form method="POST" action="{{ route('joystick.store') }}" enctype="multipart/form-data">
        @csrf
