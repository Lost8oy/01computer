<x-admin.wrapper>
    ...
    <div>
        <x-admin.breadcrumb href="{{route('joystick.index')}}" title="{{ __('Update Joystick') }}">{{ __('<< Back to all joysticks') }}</x-admin.breadcrumb>
        <x-admin.form.errors/>
    </div>
    <div class="w-full py-2 bg-white overflow-hidden">

        <form method="POST" action="{{ route('joystick.update', $joystick->id) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="py-2">
            <img src="{{ asset($joystick->icon) }}" width="200px" style="display: block;
            margin: 0 auto;">
        ...
        <x-admin.form.input id="model" class="{{$errors->has('model') ? 'border-red-400' : ''}}"
                            type="text"
                            name="model"
                            value="{{ old('model', $joystick->model) }}"
                            />
        </div>
        ...
        <div class="py">
            ...
            <select id="manufacturer_id" name="manufacturer_id" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full">
                <option value="{{ old('manufacturer_id', $joystick->manufacturer_id) }}">{{$joystick->manufacturers->name}}</option>
        ...
        <x-admin.form.input id="serial_number" class="{{$errors->has('serial_number') ? 'border-red-400' : ''}}"
                            type="text"
                            name="serial_number"
                            value="{{ old('serial_number', $joystick->serial_number) }}"
                            />
        </div>
        ...
        <x-admin.form.input id="description" class="{{$errors->has('description') ? 'border-red-400' : ''}}"
                            type="text"
                            name="description"
                            value="{{ old('description', $joystick->description) }}"
                            />
        </div>
        ...
        <x-admin.form.input id="inventory_number" class="{{$errors->has('inventory_number') ? 'border-red-400' : ''}}"
                            ...
                            value="{{ old('inventory_number', $joystick->inventory_number) }}"
                            />
        </div>
        ...
        <div class="py-2">
            <label for="bool_position" class="inline-flex items-center">
                <select id="bool_position" name="bool_position" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full">
                    <option id="1" value="{{ old('bool_position', $joystick->bool_position) }}">{{$joystick->bool_position ? 'Container' : 'Shelf'}}</option>
        ...
        <div class="py-2">
            ...
            <select id="position_select" name="position_id" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full">
                <option value="{{ old('position_id', $joystick->position_id) }}">{{ $joystick->bool_position ? $joystick->containers->code : $joystick->shelves->code }}</option>
            </select>
        </div>
